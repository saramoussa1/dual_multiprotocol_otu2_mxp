 ...
 * @author		m_richa
 * @date		03/22/19
 * @version 	1.0.5	Modified for IPGLB code style
    					Added mailbox
    					s_task 3-level priority added
...
/*local prototypes*/
static void s_task_idle(s_task_handle_t me, s_task_msg_t** context);
...
static void s_task_resume_node(s_task_node_t* node);
#ifdef USE_MESSAGING
...
static void s_task_extract_msg(s_task_handle_t s_task_dst_hndl,
							   s_task_msg_t** msg_head);
#endif
...
bool scheduler_init(ticks_cllbk_t tks_cllbk)
{
	...
	/*create idle s_task*/
	return (s_task_create(1, S_TASK_HIGH_PRIORITY, 1, s_task_idle,
						  &idle_s_task_hndl));
}
...
void scheduler(void)
{
	...
	{
		/*scan priorities*/
		for(prior_cycle_index = 0; prior_cycle_index <= PRIORITIES_CYCLE_LEN;
			 prior_cycle_index++)
		{
			...
			while(cs_task)	/*scan s_tasks nodes*/
			{
				if(cs_task->active) /*is it active?*/
				{
					...
					{
						/*check priority*/
						if(priority_cycle[prior_cycle_index] ==
							cs_task->s_priority)
						{
							/* Take a snapshot of system ticks just to
							 * make sure value does not change during this
							 * process
							 * */
							csys_tick_counter   = (*get_system_ticks)();
							ticks_diff 			= csys_tick_counter -
												  cs_task->n_ticks;

							if(ticks_diff >= cs_task->s_ticks) /*should run?*/
							{
	...
	#ifdef USE_MESSAGING
								/*msg_head =
								 * s_task_extract_msg(cs_task->s_task_id);*/
								s_task_extract_msg(cs_task->s_task_id,
												   &(cs_task->s_task_msg));
	#endif
								/*prepare call:*/
								...
								/*run task*/
								(*cs_task->s_task)(cs_task->s_task_id,
												   &(cs_task->s_task_msg));
								/*calculate elapsed ticks*/
								cs_task->e_ticks = (*get_system_ticks)() -
													cs_task->e_ticks;
	#ifdef USE_BUSY_LED
								BLEDOFF();
	#endif
 ...
 * @param[in]		s_ticks periodic ticks
 ...
 * @return false: fail, true: success
 */
bool s_task_create(bool active, s_task_priority_t s_task_prio,
		           cntr_size_t s_ticks,s_task_t s_task_ptr,
		           s_task_handle_t* s_tsk_hndl_ptr)
{
 ...
 * @return false: fail, true: success 
 */
bool s_task_resume(s_task_handle_t s_task_hndl)
{
	bool 			res  = false;
	s_task_node_t* 	node = s_task_locate(s_task_hndl);
                     ...
                     uint16_t bytes_count)
{
	...
	if(msg_node != NULL)
	{
		...
		msg_node->data 				= malloc(sizeof(bytes_count));
...
{
	node->active  = 1;
	node->n_ticks = (*get_system_ticks)() - node->s_ticks;
}
 ...
 */
static void s_task_idle(s_task_handle_t me, s_task_msg_t** msg)
{
	...
	if(((*get_system_ticks)() - init_ticks) >= TICKSPERSECOND) /*1 s elapsed?*/
	{
		...
		e_ticks_total = (e_ticks_total > TICKSPERSECOND) ?
						 TICKSPERSECOND : e_ticks_total;

		/*calculate CPU usage:*/
		cpu_usage_p = (uint8_t)(((float)e_ticks_total /
								 (float)TICKSPERSECOND) * 100.0);
